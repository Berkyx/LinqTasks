...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            var emps = Emps.Select(emp => new { emp.Ename, emp.Job, emp.HireDate });
            var noValue = new[] { new { Ename = "Brak wartości", Job = (string)null, HireDate = (DateTime?)null } };
            return emps.Union(noValue);
        }
        ...
        {
            return Depts.Where(dept => Emps.Count(emp => emp.Deptno == dept.Deptno) > 1)
                .Select(dept => new { name = dept.Dname, numOfEmployees = Emps.Count(emp => emp.Deptno == dept.Deptno) });
        }
        ...
        {
            return arr.GroupBy(n => n).Single(group => group.Count() % 2 != 0).Key;
        }
        ...
        {
            return Depts.Where(dept => Emps.Count(emp => emp.Deptno == dept.Deptno) == 5 || Emps.All(emp => emp.Deptno != dept.Deptno)).OrderBy(dept => dept.Dname);
        }
        ...
        public static IEnumerable<object> Task15()
        {
            return Emps.Where(emp => emp.Job.Contains("A"))
                ...
                .OrderByDescending(result => result.LiczbaPracownikow);
        }
        ...
        {
            return Depts.SelectMany(dept => Emps, (dept, emp) => new { Dept = dept, Emp = emp });
        }
    }
